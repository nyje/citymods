...
minetest.register_on_joinplayer(function(player)
	...
	local bal=get_money(name)
	if bal>0 then
		local coin = math.floor(bal/90)
		if coin > 0 then
			local rem = bal - (coin * 90)
			local stack = ItemStack("bitchange:minecoin "..coin)
			if player_inv:room_for_item("main", stack) then
				print("coin:"..coin.." leaving:$"..rem)
				player_inv:add_item("main", stack)
				set_money(name,rem)
				minetest.chat_send_player(name, "********* "..name..", I converted $"..(coin*90).." old money into "..coin.." new minecoins in your inventory")
			end
		end
	end
	--money.hud_add(name)
