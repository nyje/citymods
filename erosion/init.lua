...
local eroding_nodes = {
    ...
    dirt_with_rainforest_litter = {"erosion:","dirt"},
    dirt_with_coniferous_litter = {"erosion:","dirt"},
	dirt_with_grass_footsteps = {"erosion:","dirt"},
	dirt_with_dry_grass = {"erosion:","dirt"},
	dirt_with_snow = {"erosion:","dirt"},
	permafrost_with_stones = {"erosion:","permafrost"},
	permafrost_with_moss = {"erosion:","permafrost"},
    permafrost = {"erosion","permafrost"},
	sand = {"erosion:","sand"},
    silver_sand = {"erosion:","silver_sand"},
...
local lntbl = {}
local erosion_materials = {--erosion products to define
	sand = {
		description = "Sand",
	...
	dirt_with_grass = {
		...
		tiles = {"default_grass.png",
                 "default_grass.png",
                 {name = "default_grass.png", tileable_vertical = false}
                 },
		groups = {},
	...
	dirt_with_coniferous_litter = {
		...
		tiles = {"default_coniferous_litter.png",
			"default_dirt.png",
			{name = "default_coniferous_litter.png",
...
--			{name = "default_dirt.png^default_snow_side.png",
	...
	permafrost_with_stones = {
		...
		tiles = {"default_permafrost.png^default_stones_side.png", "default_permafrost.png",
			{name = "default_permafrost.png^default_stones.png",
--			{name = "default_dirt.png^default_snow_side.png",
				tileable_vertical = false}},
		groups = {},
...
local function orient_pile(k,m,p)
    local _,t0 = get_adjacent_nodes(p,{"air"})
	local a,t1 = get_adjacent_nodes(p,snt1)
	local _,t2 = get_adjacent_nodes(p,snt2)
	local x1 = t0[p.x-1] and t0[p.x-1][p.z] and 0 or t1[p.x-1] and t1[p.x-1][p.z] and 1 or t2[p.x-1] and t2[p.x-1][p.z] and 2 or 3
    local z1 = t0[p.x] and t0[p.x][p.z-1] and 0 or t1[p.x] and t1[p.x][p.z-1] and 1 or t2[p.x] and t2[p.x][p.z-1] and 2 or 3
    local x2 = t0[p.x+1] and t0[p.x+1][p.z] and 0 or t1[p.x+1] and t1[p.x+1][p.z] and 1 or t2[p.x+1] and t2[p.x+1][p.z] and 2 or 3
    local z2 = t0[p.x] and t0[p.x][p.z+1] and 0 or t1[p.x] and t1[p.x][p.z+1] and 1 or t2[p.x] and t2[p.x][p.z+1] and 2 or 3
	local d = m>3 and 0
...
function erosionCL(p,node) if not eroding_lut[node.name] then return end
	local rmnn = eroding_nodes[eroding_lut[node.name]][1].."slope_"..eroding_lut[node.name]
	local flmt = "erosion:fall_"..eroding_nodes[eroding_lut[node.name]][2]
	...
	if xpsr[1] then
		...
		if xr+zr < 7 then
            minetest.swap_node(p,{name=rmnn.."_half_raised",param2=(xr+zr-1)%4})
		elseif xr+zr < 13 then
            minetest.swap_node(p,{name=rmnn.."_cut",param2=xr>4 and zr/2-2 or (5-zr/2)%4})
		else
            minetest.swap_node(p,{name=rmnn.."_inner_cut_half_raised",param2=xr>0 and 9-zr/2 or (zr/2)%4})
        end
	end
end
