...
newnode = cityscape.clone_node("fire:permanent_flame")
newnode.description = "Molotov flames"
...
minetest.register_node("cityscape:flames", newnode)
		...
		for dy=radius,-radius,-1 do
			for dz=-radius,radius do
				...
				if n ~= "air" and na == "air" then
					...
					--minetest.place_node(pa, {name='cityscape:flames'})
					minetest.set_node(pa, {name='cityscape:flames'})
                    local t = minetest.get_node_timer(pa)
                    t:start(flamelen)
				end
			end
		end
...
local function throw_cocktail(item, player)
	...
	local obj = minetest.add_entity({x=playerpos.x,y=playerpos.y+2.625,z=playerpos.z}, 'cityscape:molotov_entity')
...
local function napalm(pos)
	...
	--minetest.place_node(pos, {name='cityscape:flames'})
    minetest.set_node(pos, {name='cityscape:flames'})
...
minetest.register_node('cityscape:molotov_cocktail', {
...
minetest.register_craft({
	output = "cityscape:gasoline",
	recipe = {
		{"", "default:steel_ingot", ""},
		{"default:steel_ingot", "farming:bottle_ethanol", "default:steel_ingot"},
		{"", "default:steel_ingot", ""},
	},
})
